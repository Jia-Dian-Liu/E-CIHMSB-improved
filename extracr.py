...
from config import Q_LENGTH, TOTAL_AVERAGES_PER_UNIT, BLOCK_SIZE, BITS_PER_AVERAGE
    ...
    for i in range(num_rows):
        ...
        for j in range(num_cols):
            ...
            for k in range(len(all_bits)):
                ...
                secret_bit = map_from_z(z_bit, cover_bit)
...
def detect_and_extract(cover_image, z_bits, contact_key=None):
    ...
    for i in range(num_rows):
        ...
        for j in range(num_cols):
            ...
            # ✨使用全部位元
            all_bits = get_all_bits_list(reordered)
            
            for k in range(len(all_bits)):
                if z_bit_index >= len(z_bits):
                    finished = True
                    break
                secret_bits.append(map_from_z(z_bits[z_bit_index], all_bits[k]))
                z_bit_index += 1
