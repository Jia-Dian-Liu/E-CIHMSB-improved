...
#sidebar-toggle-label {
    ...
    padding: clamp(10px, 1.2vw, 16px) clamp(5px, 0.6vw, 8px);
...
.welcome-container {
    ...
    margin-top: clamp(-4rem, -4vh, -1rem);
}
...
.anim-flow .anim-icon-arrow {
    width: clamp(30px, 6vmin, 80px) !important;
    height: clamp(30px, 6vmin, 80px) !important;
}
...
.anim-title {
    ...
    font-size: clamp(22px, 4.5vmin, 55px);
    font-weight: bold;
    color: #FFFFFF;
...
.anim-desc {
    font-size: clamp(16px, 3.5vmin, 45px);
...
.success-box {
    ...
    margin: 10px 0; display: inline-block; font-size: clamp(20px, 2vw, 28px); min-width: 350px;
}
.info-box {
    background: linear-gradient(135deg, #4A6B8A 0%, #5C8AAD 100%);
    color: white; padding: 20px 30px; border-radius: 10px;
    margin: 10px 0; display: inline-block; font-size: clamp(18px, 1.8vw, 26px); line-height: 1.9; min-width: 350px;
}
.info-tip-box {
    background: linear-gradient(135deg, #5C8AAD 0%, #7BA3C4 100%);
    color: white; padding: 20px 30px; border-radius: 10px;
    margin: 10px 0; display: inline-block; font-size: clamp(18px, 1.8vw, 26px); min-width: 350px;
}
.error-box {
...
/* 放大 selectbox, radio, text_area 字體 */
...
@media (max-width: 480px) {
    ...
    .anim-title { font-size: 20px !important; }
    .anim-desc { font-size: 16px !important; }
    .anim-flow img { width: 35px !important; height: 35px !important; }
    .anim-flow .anim-icon-arrow { width: 28px !important; height: 28px !important; }
    .footer-credits { font-size: 14px !important; }
}

/* 極小高度螢幕（高度 < 600px）- 防止元素超出畫面 */
...
if st.session_state.current_mode is None:
    ...
    if st.session_state.embed_page == 'result' and st.session_state.embed_result and st.session_state.embed_result.get('success'):
        ...
        #btn-back-home { position: fixed !important; bottom: 5px !important; right: 30px !important; z-index: 1000 !important; background: white !important; color: #333 !important; border: 2px solid #ccc !important; border-radius: 8px !important; cursor: pointer !important; }
        </style>
        """, unsafe_allow_html=True)
...
(function() {
    ...
    function fixAllSelectboxes() {
        ...
        captions.forEach(cap => {
            cap.style.setProperty('font-size', '22px', 'important');
        });
...
""", height=0)
        ...
        <div style="display: flex; justify-content: space-between; margin-bottom: 20px;">
            <div style="flex: 1; text-align: center; padding: 15px 10px; border-bottom: {'4px solid #4A6B8A' if current_step == 1 else '2px solid #B8C8D8'}; color: {'#4A6B8A' if current_step == 1 else '#7A8A9A'}; font-size: clamp(16px, 1.6vw, 24px); font-weight: 700;">第一步: 選擇對象</div>
            <div style="flex: 1; text-align: center; padding: 15px 10px; border-bottom: {'4px solid #4A6B8A' if current_step == 2 else '2px solid #B8C8D8'}; color: {'#4A6B8A' if current_step == 2 else '#7A8A9A'}; font-size: clamp(16px, 1.6vw, 24px); font-weight: 700;">第二步: 機密內容</div>
            <div style="flex: 1; text-align: center; padding: 15px 10px; border-bottom: {'4px solid #4A6B8A' if current_step == 3 else '2px solid #B8C8D8'}; color: {'#4A6B8A' if current_step == 3 else '#7A8A9A'}; font-size: clamp(16px, 1.6vw, 24px); font-weight: 700;">第三步: 載體圖像</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.session_state.embed_step == 1:
            ...
            if contact_names:
                ...
                if selected_contact != "選擇":
                    ...
                    st.markdown(f'<p style="font-size: clamp(18px, 1.8vw, 26px); color: #31333F;">✅ 已選擇：{selected_contact}</p>', unsafe_allow_html=True)
                    show_next_btn = True
                    next_step = 2
            else:
            ...
            st.markdown('<p style="font-size: clamp(18px, 1.8vw, 26px); font-weight: bold; margin-bottom: 5px;">內容類型</p>', unsafe_allow_html=True)
            ...
            if embed_secret_type == "文字":
                ...
                if embed_text_raw:
                    ...
                    st.markdown(f'<p style="font-size: clamp(16px, 1.6vw, 24px); color: #443C3C;"><b>機密文字:</b> {chinese} 中文 + {len(embed_text) - chinese} 英文/符號 | {secret_bits_needed:,} bits</p>', unsafe_allow_html=True)
                ...
                if embed_image:
                    ...
                    with col_info:
                        st.markdown(f'<div style="display: flex; align-items: center; min-height: 120px;"><div style="font-size: clamp(16px, 1.5vw, 22px); color: #443C3C; margin-left: 50px;"><b>機密圖像:</b> {filename} ({secret_img.size[0]}×{secret_img.size[1]} px) | {secret_bits_needed:,} bits</div></div>', unsafe_allow_html=True)
            ...
            if secret_bits_needed > 0 and selected_contact != "選擇":
                ...
                if images:
                    ...
                    with col_info:
                        if usage > 90:
                            st.markdown(f'<div style="display: flex; align-items: center; min-height: 180px;"><div style="color: #ffa726; font-size: clamp(16px, 1.5vw, 22px); margin-left: 50px;">機密容量 {secret_bits_needed:,} bits / 圖像容量 {capacity:,} bits ({usage:.1f}%)</div></div>', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<div style="display: flex; align-items: center; min-height: 180px;"><div style="font-size: clamp(16px, 1.5vw, 22px); color: #443C3C; margin-left: 50px;">機密容量 {secret_bits_needed:,} bits / 圖像容量 {capacity:,} bits ({usage:.1f}%)</div></div>', unsafe_allow_html=True)
    ...
    if st.session_state.extract_page == 'result' and st.session_state.extract_result and st.session_state.extract_result.get('success'):
        ...
        with c1:
            st.markdown(f'<div class="success-box" style="padding: 12px 20px; min-width: 200px; font-size: 24px;">提取成功! ({r["elapsed_time"]:.2f} 秒)</div>', unsafe_allow_html=True)
            ...
            if r['type'] == 'text':
                ...
                if st.button("驗證", key="verify_btn"):
                    if verify_input:
                        ...
                        with col_orig:
                            st.markdown('<p style="font-size: 20px; font-weight: bold; margin-bottom: 0;">原始機密：</p>', unsafe_allow_html=True)
                            st.markdown(f'<p style="font-size: 18px; color: #443C3C; white-space: pre-wrap; margin: 5px 0;">{verify_input}</p>', unsafe_allow_html=True)
                            st.markdown(f'<p style="font-size: 18px; color: #443C3C; margin: 0;">{len(verify_input)} 字元</p>', unsafe_allow_html=True)
                        with col_ext:
                            st.markdown('<p style="font-size: 20px; font-weight: bold; margin-bottom: 0;">提取結果：</p>', unsafe_allow_html=True)
                            st.markdown(f'<p style="font-size: 18px; color: #443C3C; white-space: pre-wrap; margin: 5px 0;">{r["content"]}</p>', unsafe_allow_html=True)
                            st.markdown(f'<p style="font-size: 18px; color: #443C3C; margin: 0;">{len(r["content"])} 字元</p>', unsafe_allow_html=True)
                        
                        if verify_input == r['content']:
                            st.markdown('<p style="font-size: 22px; font-weight: bold; color: #2E7D32; margin-top: 10px;">完全一致！</p>', unsafe_allow_html=True)
                        else:
                            st.markdown('<p style="font-size: 22px; font-weight: bold; color: #C62828; margin-top: 10px;">不一致！</p>', unsafe_allow_html=True)
                    else:
                        st.warning("請輸入原始機密")
            else:
                ...
                if verify_img:
                    ...
                    with col_orig:
                        st.markdown('<p style="font-size: 24px; font-weight: bold;">原始圖片：</p>', unsafe_allow_html=True)
                        st.image(orig_img, width=180)
                        st.markdown(f'<p style="font-size: 22px; color: #443C3C; white-space: nowrap;">尺寸：{orig_img.size[0]}×{orig_img.size[1]} | 模式：{orig_mode}</p>', unsafe_allow_html=True)
                    with col_ext:
                        st.markdown('<p style="font-size: 24px; font-weight: bold;">提取結果：</p>', unsafe_allow_html=True)
                        st.image(extracted_img, width=180)
                        st.markdown(f'<p style="font-size: 22px; color: #443C3C; white-space: nowrap;">尺寸：{extracted_img.size[0]}×{extracted_img.size[1]} | 模式：{ext_mode}</p>', unsafe_allow_html=True)
                    ...
                    if orig_arr.shape == ext_arr.shape:
                        ...
                        mse = np.mean(diff ** 2)
                        if mse == 0:
                            st.markdown(f'<p style="font-size: 24px; color: #443C3C;">MSE: {mse:.4f} &nbsp;&nbsp; <b style="color: #2E7D32;">完全一致！</b></p>', unsafe_allow_html=True)
                        elif mse < 100:
                            st.markdown(f'<p style="font-size: 24px; color: #443C3C;">MSE: {mse:.4f} &nbsp;&nbsp; <b style="color: #F57C00;">接近一致</b></p>', unsafe_allow_html=True)
                        else:
                            st.markdown(f'<p style="font-size: 24px; color: #443C3C;">MSE: {mse:.4f} &nbsp;&nbsp; <b style="color: #C62828;">不一致</b></p>', unsafe_allow_html=True)
                    else:
                        st.markdown(f'<p style="font-size: 24px; color: #1976D2;">尺寸不同：原始 {orig_img.size} vs 提取 {extracted_img.size}</p>', unsafe_allow_html=True)
        
        st.markdown("""
        <style>
        #btn-back-home-extract span, #btn-back-home-extract p { font-size: 18px !important; font-weight: bold !important; }
        #btn-back-home-extract { position: fixed !important; bottom: 5px !important; right: 30px !important; z-index: 1000 !important; background: white !important; color: #333 !important; border: 2px solid #ccc !important; border-radius: 8px !important; cursor: pointer !important; }
...
""", height=0)
        ...
        st.markdown(f"""
        <div style="display: flex; justify-content: space-between; margin-bottom: 20px;">
            <div style="flex: 1; text-align: center; padding: 15px 10px; border-bottom: {'4px solid #7D5A6B' if current_step == 1 else '2px solid #D8C0C8'}; color: {'#7D5A6B' if current_step == 1 else '#A08090'}; font-size: clamp(16px, 1.6vw, 24px); font-weight: 700;">第一步: 選擇對象</div>
            <div style="flex: 1; text-align: center; padding: 15px 10px; border-bottom: {'4px solid #7D5A6B' if current_step == 2 else '2px solid #D8C0C8'}; color: {'#7D5A6B' if current_step == 2 else '#A08090'}; font-size: clamp(16px, 1.6vw, 24px); font-weight: 700;">第二步: 上傳 Z碼圖</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.session_state.extract_step == 1:
            ...
            if contact_names:
                ...
                if selected_contact != "選擇":
                    ...
                    st.markdown(f'<p style="font-size: clamp(18px, 1.8vw, 26px); color: #31333F;">✅ 已選擇：{selected_contact}（{selected_style}）</p>', unsafe_allow_html=True)
                    show_next_btn = True
                    next_step = 2
            else:
                st.markdown("""<div style="background: linear-gradient(135deg, #fff3cd 0%, #ffe69c 100%); border: 2px solid #ffc107; border-radius: 12px; padding: 15px; text-align: center; margin: 10px 0;"><div style="font-size: 16px; font-weight: bold; color: #856404;">⚠️ 請先新增對象（點擊左上角「對象管理」按鈕）</div></div>""", unsafe_allow_html=True)
            ...
            if saved_contact and saved_contact in contact_names:
                ...
                st.markdown(f'<p style="font-size: clamp(16px, 1.6vw, 24px); color: #31333F;">對象：{selected_contact}（{saved_style}）</p>', unsafe_allow_html=True)
                
                st.markdown('<p style="font-size: clamp(18px, 1.8vw, 26px); font-weight: bold; margin-bottom: 10px;">上傳 Z碼圖</p>', unsafe_allow_html=True)
                ...
                if extract_file:
                    ...
                    with col_info:
                        if detected:
                            st.markdown(f'<div style="display: flex; align-items: center; min-height: 180px;"><div style="font-size: clamp(16px, 1.5vw, 22px); color: #443C3C; margin-left: 50px;">{success_msg.replace("<br>", " ")}</div></div>', unsafe_allow_html=True)
                        else:
                            st.markdown('<div style="display: flex; align-items: center; min-height: 180px;"><div style="font-size: clamp(16px, 1.5vw, 22px); color: #C62828; margin-left: 50px;">無法識別，請確認上傳的是 QR Code 或 Z碼圖</div></div>', unsafe_allow_html=True)
            else:
                st.info("請先完成步驟 1（選擇對象）")
