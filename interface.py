        ...
        with col_right:
            ...
            if r['type'] == 'text':
                ...
                if 'verify_result' in st.session_state and st.session_state.verify_result:
                    ...
                    col_orig, col_gap, col_ext = st.columns([1, 0.2, 1])
                    with col_orig:
                        st.markdown('<p style="font-size: 16px; color: #443C3C;"><b>原始輸入：</b></p>', unsafe_allow_html=True)
                        st.markdown(f'<p style="font-size: 14px; color: #666; white-space: pre-wrap; line-height: 1.8; word-break: keep-all;">{vr["input"]}</p>', unsafe_allow_html=True)
                    with col_ext:
                        st.markdown('<p style="font-size: 16px; color: #443C3C;"><b>提取結果：</b></p>', unsafe_allow_html=True)
                        st.markdown(f'<p style="font-size: 14px; color: #666; white-space: pre-wrap; line-height: 1.8; word-break: keep-all;">{r["content"]}</p>', unsafe_allow_html=True)
            else:
